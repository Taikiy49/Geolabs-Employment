...
function StepReview({ form, resumeFile, onReset }) {
  ...
  // Warning-only list (never blocks submission)
  ...
  const payload = useMemo(() => {
    return {
      ...
      legalText: {
        ...
        eeoNotice: EEO_NOTICE_TEXT,
        disabilityNotice: DISABILITY_NOTICE_TEXT,
        veteranNotice: VETERAN_NOTICE_TEXT,
      },
      clientMeta: {
  ...
  // ✅ Only requirement to proceed: confirm checkbox
  const canSubmit = !submitting && !submitState.ok && confirmChecked;
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      if (resumeFile instanceof File) fd.append("resume", resumeFile, resumeFile.name);
  ...
  return (
    <section className="form-section review-section">
      <div className="review-header">
        <div>
          ...
          <p className="section-help">
            Please confirm the information below. When you submit, your application will be securely sent to Geolabs, Inc.
          </p>
        </div>
      ...
      {/* Warning banner (does not block) */}
      {missingCritical.length > 0 && !submitState.ok && (
        <div className="review-banner review-banner-warn" role="status">
          <strong>Missing items:</strong> {missingCritical.join(", ")}.
          <div className="review-warn-sub">You can still submit — you’ll confirm again in the final prompt.</div>
        </div>
      )}
      ...
      <div className="review-grid">
          ...
          <dl className="review-list">
            ...
            <div className="review-list-row">
              ...
              <dd className={isEmpty(form?.email) && isEmpty(form?.phone) ? "review-empty" : ""}>
                {form?.email || "—"}
                {form?.phone ? <span className="review-contact-secondary"> / {form.phone}</span> : null}
              </dd>
            </div>
            <div className="review-list-row">
              <dt>Resume</dt>
        ...
        <div className="review-card">
          <div className="review-card-header">
            ...
            <span className="review-chip">Voluntary</span>
          </div>

          <dl className="review-list">
            <div className="review-list-row">
              <dt>Veteran Status</dt>
              <dd className={isEmpty(form?.vetStatus) ? "review-empty" : ""}>{formatVetStatus()}</dd>
            </div>
          </dl>

          <p className="review-note">
            These responses are voluntary and used only for reporting and compliance purposes.
          </p>
        </div>
        ...
        <div className="review-card">
          <div className="review-card-header">
            ...
            <span className="review-chip review-chip-attestation">Attestation</span>
          </div>
      ...
      <div className="review-final">
        <div className="review-final-card">
          <div className="review-final-top">
            <div>
              ...
              <p className="review-final-sub">
                Check the box to confirm, then submit. You’ll see a final “Are you sure?” prompt.
              </p>
            </div>

            <span className={"review-status-tag" + (canSubmit ? " ready" : "")}>
              {submitState.ok ? "Submitted" : canSubmit ? "Ready to submit" : "Not ready"}
            </span>
          </div>
          ...
          <label className="review-confirm">
            ...
            />
            <span>
              I confirm the information above is accurate to the best of my knowledge and I want to submit this application.
            </span>
          </label>
          ...
          <div className="review-actions">
            ...
            {submitState.ok && typeof onReset === "function" && (
              <button type="button" className="review-btn review-btn-ghost" onClick={onReset}>
          ...
          {!confirmChecked && !submitState.ok && (
            <p className="review-minihelp">Check the confirmation box to enable submission.</p>
          )}
        </div>
      </div>

      {/* Confirm modal (your “extra check”) */}
      {modalOpen && (
        <div
          ...
          aria-modal="true"
          onMouseDown={(e) => {
            if (e.target.classList.contains("review-modalOverlay")) closeModal();
          }}
        >
          <div className="review-modal">
            ...
            {missingCritical.length > 0 && (
              <div className="review-modal-warnBox" role="status">
                <div className="review-modal-warnTitle">Incomplete submission warning</div>
                <div className="review-modal-warnBody">
                  You are about to submit without the following item(s):
                  <div className="review-modal-missing">{missingCritical.join(", ")}.</div>
                  Please confirm you still want to submit.
                </div>
              </div>
            )}
            ...
            <div className="review-modal-actions">
              ...
              <button
                ...
                className={
                  "review-btn " +
                  (missingCritical.length > 0 ? "review-btn-danger" : "review-btn-primary")
                }
                onClick={doSubmit}
                disabled={submitting}
              >
